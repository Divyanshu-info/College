...
void split(vector<vector<int>> &A, vector<vector<int>> &B, int row, int col,
           int d) {
...
void join(vector<vector<int>> &A, vector<vector<int>> &B, int row, int col,
          int d) {
  for (int i1 = 0, i2 = row; i1 < d; i1++, i2++) {
    for (int j1 = 0, j2 = col; j1 < d; j1++, j2++) {
      B[i2][j2] = A[i1][j1];
...
int main(int argc, char *argv[]) {
  ...
  if (pid == 0) {
    ...
    cout << "\rNaive Matrix Multiplication took: " << duration1
         << " microseconds.\n";
    ...
    cout << "\rStrassen's Matrix Multiplication took: " << duration2
         << " microseconds.\n";
    kill(pid, SIGKILL);
    return 0;
  }
